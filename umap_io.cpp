...
class UmapIO : public FileIO {
    public:
        ...
        void Write(std::string fn , size_t bs, size_t p, uint32_t wcnt, uint32_t rcnt ) {
            ...
            void* base_addr = umap(NULL, p, PROT_READ|PROT_WRITE, UMAP_PRIVATE, fd, 0);
            ...
            memcpy(base, wbuffer, p);
            msync(base, p, MS_SYNC);


            if (uunmap(base_addr, p) < 0) {
                int eno = errno;
            ...
            std::chrono::steady_clock::time_point end = std::chrono::steady_clock::now();
            double time_taken = (std::chrono::duration_cast<std::chrono::microseconds>(end - begin).count()) / 1000000.0;
            std::cout << "UMap Write\t" << p/KB/KB << "MB\t" << time_taken << "s" << std::endl;
            close(fd);
        }
